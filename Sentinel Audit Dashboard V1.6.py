# -*- coding: utf-8 -*-
"""
Sentinel Audit Dashboard V1.6 - Enhanced Version with AI, Technician Management & Email Reports
Created on Mon Oct 13 13:21:52 2025
@author: charl

New Features in V1.6:
- Technician-Site Assignment System: Manage technicians and assign them to sites
- AI Query Agent: Natural language queries using Anthropic Claude API
- Automated Daily Email Reports: Scheduled email reports sent at 5 AM daily
...
def get_script_path():
    """Gets the directory where the executable/script resides - permanent location"""
...
def load_config():
    """Load application configuration including API keys and email settings"""
    config_path = os.path.join(get_script_path(), CONFIG_FILENAME)
    default_config = {
        "anthropic_api_key": "",
        "email": {
    ...
    if os.path.exists(config_path):
        try:
            ...
            log_message(f"Error loading config: {err}", "ERROR")
            return default_config
    else:
...
def save_config(config_dict):
    ...
    try:
        ...
        log_message(f"Error saving config: {err}", "ERROR")
        # Try to restore from backup
        backup_path = config_path + '.backup'
        if os.path.exists(backup_path):
            shutil.copy2(backup_path, config_path)
            log_message("Restored config from backup", "WARNING")
...
class SentinelDashboard(tk.Tk):
    def __init__(self):
        ...
        # Setup UI
        self.title("Sentinel Audit Dashboard V1.6")
        ...
        if self.config.get('email_schedule', {}).get('enabled'):
    ...
    def setup_ui(self):
        ...
        # Title Label
        title_label = tk.Label(header_inner, text="Sentinel Audit Dashboard V1.6",
        ...
        self.btn_technicians = tk.Button(control_frame, text="👥 Technicians",
                                         bg='#8e44ad', fg='white',
                                         activebackground='#7d3c98',
                                         command=self.show_technician_management, **btn_style)
        self.btn_technicians.pack(side="left", padx=(0, 10))

        self.btn_ai_chat = tk.Button(control_frame, text="🤖 AI Assistant",
                                     bg='#16a085', fg='white',
                                     activebackground='#138d75',
                                     command=self.show_ai_chat, **btn_style)
        self.btn_ai_chat.pack(side="left", padx=(0, 10))

        self.btn_email_config = tk.Button(control_frame, text="✉️ Email Config",
                                          bg='#d35400', fg='white',
                                          activebackground='#ba4a00',
                                          command=self.show_email_config, **btn_style)
        self.btn_email_config.pack(side="left")
    ...
    def view_selected_equipment_notes(self):
        """Wrapper to view notes - shows all equipment with notes or specific equipment"""
        selected_items = self.dashboard_tree.selection()
        
        if not selected_items:
            # No selection - show all equipment with notes
            self.show_all_equipment_with_notes()
        else:
            # Has selection - show notes for that equipment
            values = self.dashboard_tree.item(selected_items[0])['values']
            if not values:
                self.show_all_equipment_with_notes()
                return
                
            equipment_id = str(values[0]).replace('📝 ', '')
            ...
            self.show_equipment_notes(equipment_id)

    def show_all_equipment_with_notes(self):
        """Display a list of all equipment that have notes"""
        try:
            with self.get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT 
                        ...
                        MAX(en.created_date) as last_note_date
                    FROM equipment_notes en
                    GROUP BY en.equipment_id
                    ORDER BY last_note_date DESC
                """)
                results = cursor.fetchall()
                
                if not results:
                    messagebox.showinfo("Equipment Notes", 
                                      "No equipment has notes yet.\n\nSelect an equipment and add notes to get started!")
                    return
        except Exception as err:
            log_message(f"Error loading equipment notes list: {err}", "ERROR")
            messagebox.showerror("Error", f"Failed to load equipment list: {err}")
            return
        
        # Create window
        notes_list_window = tk.Toplevel(self)
        notes_list_window.title(f"Equipment with Notes - {self.selected_shaft.get()}")
        notes_list_window.geometry("800x600")
        notes_list_window.configure(bg=self.colors['bg'])
        
        # Header
        header_frame = tk.Frame(notes_list_window, bg='white', relief=tk.FLAT, bd=0)
        header_frame.pack(fill='x', padx=0, pady=0)
        ...
        title_label = tk.Label(header_inner, 
                              text=f"📝 Equipment with Notes ({len(results)} total)", 
                              font=('Segoe UI', 14, 'bold'), 
                              bg='white', fg=self.colors['primary'])
        title_label.pack(side="left")
        
        separator = tk.Frame(notes_list_window, height=2, bg='#ddd')
        separator.pack(fill='x')
        
        # Content frame
        content_frame = tk.Frame(notes_list_window, bg=self.colors['bg'])
        content_frame.pack(fill='both', expand=True, padx=20, pady=20)
        
        # Search frame
        search_frame = tk.Frame(content_frame, bg='white', relief=tk.FLAT, bd=0)
        search_frame.pack(fill='x', pady=(0, 10))
        
        search_inner = tk.Frame(search_frame, bg='white')
        search_inner.pack(fill='x', padx=15, pady=10)
        
        search_label = tk.Label(search_inner, text="🔍 Search:", 
                               font=('Segoe UI', 9, 'bold'),
                               bg='white', fg=self.colors['text'])
        search_label.pack(side='left', padx=(0, 10))
        ...
        # List frame
        list_frame = tk.Frame(content_frame, bg='white', relief=tk.FLAT, bd=0)
        ...
        style = ttk.Style()
        style.configure('NotesList.Treeview',
                       background='white',
                       foreground=self.colors['text'],
                       fieldbackground='white',
                       font=('Segoe UI', 9),
                       rowheight=25)
        style.configure('NotesList.Treeview.Heading',
                       background=self.colors['primary'],
                       foreground='white',
                       font=('Segoe UI', 10, 'bold'))
        
        columns = ('equipment_id', 'note_count', 'last_note')
        notes_tree = ttk.Treeview(list_frame,
                                 columns=columns,
                                 show='headings',
                                 yscrollcommand=list_scroll.set,
                                 style='NotesList.Treeview')
        list_scroll.config(command=notes_tree.yview)
        
        notes_tree.heading('equipment_id', text='Equipment ID')
        notes_tree.heading('note_count', text='# of Notes')
        notes_tree.heading('last_note', text='Last Note Date')
        
        notes_tree.column('equipment_id', width=300, anchor='w')
        notes_tree.column('note_count', width=120, anchor='center')
        notes_tree.column('last_note', width=200, anchor='center')
        ...
        all_data = []
        for row in results:
            equipment_id, note_count, last_date = row
            try:
                dt = datetime.strptime(last_date, DATETIME_FORMAT)
                formatted_date = dt.strftime('%Y-%m-%d %H:%M')
            except:
                formatted_date = last_date if last_date else 'N/A'
            
            all_data.append((equipment_id, note_count, formatted_date))
        
        def populate_tree(filter_text=''):
            """Populate tree with optional filtering"""
            notes_tree.delete(*notes_tree.get_children())
            
            filter_text = filter_text.lower().strip()
            
            for equipment_id, note_count, formatted_date in all_data:
                if not filter_text or filter_text in equipment_id.lower():
                    notes_tree.insert('', 'end', 
                                    values=(equipment_id, note_count, formatted_date),
                                    tags=('clickable',))
            
            # Update title with count
            visible_count = len(notes_tree.get_children())
            title_label.config(text=f"📝 Equipment with Notes ({visible_count} of {len(results)})")
        ...
        def on_double_click(event):
            selected = notes_tree.selection()
            if selected:
                item = notes_tree.item(selected[0])
                equipment_id = item['values'][0]
                notes_list_window.destroy()
                self.show_equipment_notes(equipment_id)
        
        notes_tree.bind('<Double-Button-1>', on_double_click)
        ...
        def open_selected():
            selected = notes_tree.selection()
            if not selected:
                messagebox.showwarning("No Selection", "Please select an equipment from the list.")
                return
        ...
        btn_open = tk.Button(button_frame, text="📝 View Notes",
        ...
        btn_close = tk.Button(button_frame, text="Close",
                             command=notes_list_window.destroy,
                             bg=self.colors['danger'], fg='white',
                             activebackground='#c0392b',
        ...
        info_label = tk.Label(button_frame, 
                             text="💡 Tip: Double-click any row to view its notes",
    ...
    def show_technician_management(self):
        """Show technician management window"""
        tech_window = tk.Toplevel(self)
        tech_window.title("Technician Management")
        tech_window.geometry("800x600")
        tech_window.configure(bg=self.colors['bg'])

        # Header
        header_frame = tk.Frame(tech_window, bg='white')
        header_frame.pack(fill='x', padx=0, pady=0)

        title_label = tk.Label(header_frame, text="👥 Technician Management",
                              font=('Segoe UI', 16, 'bold'),
                              bg='white', fg=self.colors['primary'])
        title_label.pack(pady=15)

        # Control buttons
        control_frame = tk.Frame(tech_window, bg=self.colors['bg'])
        control_frame.pack(fill='x', padx=20, pady=10)

        btn_style = {'font': ('Segoe UI', 9), 'relief': tk.FLAT, 'bd': 0,
                    'padx': 15, 'pady': 8, 'cursor': 'hand2'}

        tk.Button(control_frame, text="➕ Add Technician",
                 bg=self.colors['success'], fg='white',
                 command=lambda: self.add_technician(tech_tree),
                 **btn_style).pack(side='left', padx=5)

        tk.Button(control_frame, text="✏️ Edit Technician",
                 bg=self.colors['secondary'], fg='white',
                 command=lambda: self.edit_technician(tech_tree),
                 **btn_style).pack(side='left', padx=5)

        tk.Button(control_frame, text="🗑️ Delete Technician",
                 bg=self.colors['danger'], fg='white',
                 command=lambda: self.delete_technician(tech_tree),
                 **btn_style).pack(side='left', padx=5)

        # Treeview for technicians
        tree_frame = tk.Frame(tech_window, bg=self.colors['bg'])
        ...
        tech_tree = ttk.Treeview(tree_frame,
                                 columns=('ID', 'Name', 'Email', 'Site', 'Employee ID'),
                                 show='headings',
                                 yscrollcommand=tree_scroll_y.set,
                                 xscrollcommand=tree_scroll_x.set)

        tree_scroll_y.config(command=tech_tree.yview)
        tree_scroll_x.config(command=tech_tree.xview)
        tree_scroll_y.pack(side='right', fill='y')
        tree_scroll_x.pack(side='bottom', fill='x')
        tech_tree.pack(fill='both', expand=True)

        # Configure columns
        tech_tree.heading('ID', text='ID')
        tech_tree.heading('Name', text='Name')
        tech_tree.heading('Email', text='Email')
        tech_tree.heading('Site', text='Site')
        tech_tree.heading('Employee ID', text='Employee ID')

        tech_tree.column('ID', width=50, anchor='center')
        tech_tree.column('Name', width=150)
        tech_tree.column('Email', width=200)
        tech_tree.column('Site', width=150)
        tech_tree.column('Employee ID', width=100)

        # Load technicians
        self.refresh_technician_list(tech_tree)

    def refresh_technician_list(self, tree):
        """Refresh the technician list in the treeview"""
        # Clear existing items
        for item in tree.get_children():
            tree.delete(item)

        try:
            # Get all technicians from all site databases
            all_technicians = []
            for site_name, db_file in self.shaft_databases_cache.items():
                db_path = os.path.join(get_script_path(), db_file)
                if os.path.exists(db_path):
                    conn = sqlite3.connect(db_path, timeout=10.0)
                    cursor = conn.cursor()
                    cursor.execute("SELECT id, name, email, site_name, employee_id FROM technicians ORDER BY name")
                    technicians = cursor.fetchall()
                    all_technicians.extend(technicians)
                    conn.close()

            # Remove duplicates based on email (assuming email is unique)
            seen = set()
            unique_technicians = []
            for tech in all_technicians:
                if tech[2] not in seen:  # tech[2] is email
                    seen.add(tech[2])
                    unique_technicians.append(tech)

            # Insert into treeview
            for tech in unique_technicians:
                tree.insert('', 'end', values=tech)

        except Exception as err:
            log_message(f"Error refreshing technician list: {err}", "ERROR")

    def add_technician(self, tree):
        """Add a new technician"""
        add_window = tk.Toplevel(self)
        add_window.title("Add Technician")
        add_window.geometry("400x350")
        add_window.configure(bg=self.colors['bg'])

        # Form fields
        fields_frame = tk.Frame(add_window, bg=self.colors['bg'])
        fields_frame.pack(padx=20, pady=20, fill='both')

        tk.Label(fields_frame, text="Name:", font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=0, column=0, sticky='w', pady=5)
        name_entry = tk.Entry(fields_frame, font=('Segoe UI', 10), width=30)
        name_entry.grid(row=0, column=1, pady=5)

        tk.Label(fields_frame, text="Email:", font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=1, column=0, sticky='w', pady=5)
        email_entry = tk.Entry(fields_frame, font=('Segoe UI', 10), width=30)
        email_entry.grid(row=1, column=1, pady=5)

        tk.Label(fields_frame, text="Site:", font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=2, column=0, sticky='w', pady=5)
        site_var = tk.StringVar()
        site_combo = ttk.Combobox(fields_frame, textvariable=site_var, values=list(self.shaft_databases_cache.keys()),
                                  state="readonly", width=28, font=('Segoe UI', 10))
        site_combo.grid(row=2, column=1, pady=5)
        if self.shaft_databases_cache:
            site_combo.current(0)

        tk.Label(fields_frame, text="Employee ID:", font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=3, column=0, sticky='w', pady=5)
        employee_entry = tk.Entry(fields_frame, font=('Segoe UI', 10), width=30)
        employee_entry.grid(row=3, column=1, pady=5)

        def save_technician():
            name = name_entry.get().strip()
            email = email_entry.get().strip()
            site = site_var.get().strip()
            employee_id = employee_entry.get().strip()

            if not name or not email or not site:
                messagebox.showwarning("Missing Information", "Please fill in Name, Email, and Site.")
                return

            try:
                # Add to the site's database
                db_file = self.shaft_databases_cache[site]
                db_path = os.path.join(get_script_path(), db_file)
                conn = sqlite3.connect(db_path, timeout=10.0)
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO technicians (name, email, site_name, employee_id)
                    VALUES (?, ?, ?, ?)
                """, (name, email, site, employee_id))
                conn.commit()
                conn.close()

                messagebox.showinfo("Success", f"Technician {name} added successfully!")
                self.refresh_technician_list(tree)
                add_window.destroy()
                log_message(f"Added technician: {name} to site {site}", "INFO")

            except Exception as err:
                messagebox.showerror("Error", f"Failed to add technician: {err}")
                log_message(f"Error adding technician: {err}", "ERROR")

        # Save button
        tk.Button(add_window, text="💾 Save", bg=self.colors['success'], fg='white',
                 font=('Segoe UI', 10), relief=tk.FLAT, bd=0, padx=20, pady=10,
                 command=save_technician).pack(pady=20)

    def edit_technician(self, tree):
        """Edit selected technician"""
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("No Selection", "Please select a technician to edit.")
            return

        item = tree.item(selected[0])
        values = item['values']
        tech_id, name, email, site, employee_id = values

        edit_window = tk.Toplevel(self)
        edit_window.title("Edit Technician")
        edit_window.geometry("400x350")
        edit_window.configure(bg=self.colors['bg'])

        # Form fields
        fields_frame = tk.Frame(edit_window, bg=self.colors['bg'])
        fields_frame.pack(padx=20, pady=20, fill='both')
        ...
        tk.Label(fields_frame, text="Employee ID:", font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=3, column=0, sticky='w', pady=5)
        employee_entry = tk.Entry(fields_frame, font=('Segoe UI', 10), width=30)
        employee_entry.insert(0, employee_id if employee_id else '')
        employee_entry.grid(row=3, column=1, pady=5)
        ...
        def update_technician():
            ...
            new_employee_id = employee_entry.get().strip()

            if not new_name or not new_email or not new_site:
                messagebox.showwarning("Missing Information", "Please fill in Name, Email, and Site.")
                return

            try:
                # Delete from old site if changed
                if new_site != site:
                    old_db_file = self.shaft_databases_cache[site]
                    old_db_path = os.path.join(get_script_path(), old_db_file)
                    conn = sqlite3.connect(old_db_path, timeout=10.0)
                    cursor = conn.cursor()
                    ...
                    # Add to new site
                    new_db_file = self.shaft_databases_cache[new_site]
                    new_db_path = os.path.join(get_script_path(), new_db_file)
                    conn = sqlite3.connect(new_db_path, timeout=10.0)
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO technicians (name, email, site_name, employee_id)
                        VALUES (?, ?, ?, ?)
                    """, (new_name, new_email, new_site, new_employee_id))
                else:
                    # Update in same site
                    db_file = self.shaft_databases_cache[site]
                    db_path = os.path.join(get_script_path(), db_file)
                    ...
                    cursor.execute("""
                        UPDATE technicians
                        SET name = ?, email = ?, site_name = ?, employee_id = ?
                        WHERE id = ?
                    """, (new_name, new_email, new_site, new_employee_id, tech_id))

                conn.commit()
                conn.close()

                messagebox.showinfo("Success", "Technician updated successfully!")
                self.refresh_technician_list(tree)
                edit_window.destroy()
                log_message(f"Updated technician: {new_name}", "INFO")

            except Exception as err:
                messagebox.showerror("Error", f"Failed to update technician: {err}")
                log_message(f"Error updating technician: {err}", "ERROR")

        # Update button
        tk.Button(edit_window, text="💾 Update", bg=self.colors['secondary'], fg='white',
                 font=('Segoe UI', 10), relief=tk.FLAT, bd=0, padx=20, pady=10,
                 command=update_technician).pack(pady=20)

    def delete_technician(self, tree):
        """Delete selected technician"""
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("No Selection", "Please select a technician to delete.")
            return
        ...
        if not messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete technician {name}?"):
            return
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Technician {name} deleted successfully!")
            self.refresh_technician_list(tree)
            log_message(f"Deleted technician: {name}", "INFO")

        except Exception as err:
            messagebox.showerror("Error", f"Failed to delete technician: {err}")
            log_message(f"Error deleting technician: {err}", "ERROR")
    ...
    def show_ai_chat(self):
        """Show AI chat interface for querying dashboard data"""
        if not ANTHROPIC_AVAILABLE:
            messagebox.showerror("AI Not Available",
                               "The anthropic package is not installed. Please install it using:\npip install anthropic")
            return

        config = load_config()
        if not config.get('anthropic_api_key'):
            messagebox.showwarning("API Key Missing",
                                 "Please configure your Anthropic API key in Email Config first.")
            return

        chat_window = tk.Toplevel(self)
        chat_window.title("AI Dashboard Assistant")
        chat_window.geometry("700x600")
        chat_window.configure(bg=self.colors['bg'])

        # Header
        header_frame = tk.Frame(chat_window, bg='white')
        header_frame.pack(fill='x', padx=0, pady=0)

        title_label = tk.Label(header_frame, text="🤖 AI Dashboard Assistant",
                              font=('Segoe UI', 16, 'bold'),
                              bg='white', fg=self.colors['primary'])
        title_label.pack(pady=15)

        # Chat display area
        chat_frame = tk.Frame(chat_window, bg=self.colors['bg'])
        ...
        # Initial message
        self._add_chat_message(chat_display, "AI",
                              "Hello! I'm your AI dashboard assistant. Ask me questions about your sensor data, "
        ...
        # Input area
        input_frame = tk.Frame(chat_window, bg=self.colors['bg'])
        input_frame.pack(fill='x', padx=20, pady=10)

        query_entry = tk.Entry(input_frame, font=('Segoe UI', 10), relief=tk.FLAT, bd=2)
        ...
        def send_query():
            ...
            self._add_chat_message(chat_display, "You", query)
            query_entry.delete(0, tk.END)

            # Process query in thread
            run_in_thread(lambda: self._process_ai_query(chat_display, query))

        query_entry.bind('<Return>', lambda e: send_query())

        tk.Button(input_frame, text="Send", bg=self.colors['secondary'], fg='white',
                 font=('Segoe UI', 10), relief=tk.FLAT, bd=0, padx=20, pady=8,
                 command=send_query).pack(side='right')

    def _add_chat_message(self, chat_display, sender, message):
        ...
        def update():
            ...
            chat_display.tag_config('sender', font=('Segoe UI', 10, 'bold'))
    ...
    def _process_ai_query(self, chat_display, query):
        """Process AI query and display response"""
        try:
            config = load_config()
            client = anthropic.Anthropic(api_key=config['anthropic_api_key'])

            # Gather context from databases
            context = self._gather_dashboard_context(query)

            # Create prompt
            prompt = f"""You are an AI assistant for a Sentinel Audit Dashboard that tracks sensor test data for mining sites.
...
Based on this data, please answer the user's question. Be concise and precise. If you need to calculate metrics, show your reasoning."""
            ...
            self._add_chat_message(chat_display, "AI", response)
            log_message(f"AI query processed: {query[:50]}...", "INFO")

        except Exception as err:
            error_msg = f"Error processing query: {err}"
            self._add_chat_message(chat_display, "AI", error_msg)
            log_message(f"AI query error: {err}", "ERROR")

    def _gather_dashboard_context(self, query):
        """Gather relevant context from databases based on query"""
        context_parts = []
        query_lower = query.lower()

        # Determine which sites to query
        sites_to_check = []
        for site_name in self.shaft_databases_cache.keys():
            if site_name.lower() in query_lower:
                sites_to_check.append(site_name)

        # If no specific site mentioned, include all sites
        if not sites_to_check:
            sites_to_check = list(self.shaft_databases_cache.keys())

        # Gather data for each site
        for site_name in sites_to_check:
            try:
                db_file = self.shaft_databases_cache[site_name]
                db_path = os.path.join(get_script_path(), db_file)
                ...
                cursor.execute("SELECT COUNT(*) FROM sensor_tests WHERE outcome = 'fail'")
                ...
                # Get technicians for this site
                cursor.execute("SELECT COUNT(*) FROM technicians WHERE site_name = ?", (site_name,))
                ...
                if 'today' in query_lower or 'daily' in query_lower:
                    ...
                    cursor.execute("""
                        SELECT COUNT(*) FROM sensor_tests
                        WHERE DATE(time_tested) = ?
                    """, (today,))
                    tests_today = cursor.fetchone()[0]

                    cursor.execute("""
                        SELECT COUNT(*) FROM sensor_tests
                        WHERE DATE(time_tested) = ? AND outcome = 'fail'
                    """, (today,))
                    failures_today = cursor.fetchone()[0]

                    context_parts.append(f"""
Site: {site_name}
- Tests Today: {tests_today}
- Failures Today: {failures_today}
- Total Tests (All Time): {total_tests}
- Total Failures (All Time): {total_failures}
- Failure Rate: {(total_failures/total_tests*100 if total_tests > 0 else 0):.1f}%
- Unique Equipment: {unique_equipment}
- Assigned Technicians: {tech_count}
...
""")
                ...
                # If asking about specific equipment
                if 'equipment' in query_lower or 'instrument' in query_lower:
                    cursor.execute("""
                        SELECT equipment_id, COUNT(*) as test_count,
                    ...
                    if top_failures:
                        context_parts.append(f"\nTop 5 Equipment with Most Failures on {site_name}:")
                        for eq_id, test_count, fail_count in top_failures:
                            context_parts.append(f"  - {eq_id}: {fail_count} failures out of {test_count} tests")

                conn.close()

            except Exception as err:
                log_message(f"Error gathering context for {site_name}: {err}", "ERROR")
                continue

        return "\n".join(context_parts) if context_parts else "No data available."

    # ----------------------------------------------------------------------
    # EMAIL CONFIGURATION AND REPORTING
    # ----------------------------------------------------------------------
    ...
    def show_email_config(self):
        ...
        config_window.title("Email & API Configuration")
        config_window.geometry("500x550")
        config_window.configure(bg=self.colors['bg'])

        # Header
        header_frame = tk.Frame(config_window, bg='white')
        header_frame.pack(fill='x', padx=0, pady=0)

        title_label = tk.Label(header_frame, text="⚙️ Configuration",
                              font=('Segoe UI', 16, 'bold'),
                              bg='white', fg=self.colors['primary'])
        title_label.pack(pady=15)

        # Form frame
        form_frame = tk.Frame(config_window, bg=self.colors['bg'])
        form_frame.pack(fill='both', expand=True, padx=20, pady=20)

        # AI API Key Section
        tk.Label(form_frame, text="🤖 AI Configuration", font=('Segoe UI', 12, 'bold'),
                bg=self.colors['bg'], fg=self.colors['primary']).grid(row=0, column=0, columnspan=2, sticky='w', pady=(0, 10))

        tk.Label(form_frame, text="Anthropic API Key:", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=1, column=0, sticky='w', pady=5)
        api_key_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35, show='*')
        api_key_entry.insert(0, config.get('anthropic_api_key', ''))
        api_key_entry.grid(row=1, column=1, pady=5)

        # Email Section
        tk.Label(form_frame, text="📧 Email Configuration", font=('Segoe UI', 12, 'bold'),
                bg=self.colors['bg'], fg=self.colors['primary']).grid(row=2, column=0, columnspan=2, sticky='w', pady=(20, 10))

        tk.Label(form_frame, text="SMTP Server:", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=3, column=0, sticky='w', pady=5)
        smtp_server_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35)
        smtp_server_entry.insert(0, config.get('email', {}).get('smtp_server', 'smtp.gmail.com'))
        smtp_server_entry.grid(row=3, column=1, pady=5)

        tk.Label(form_frame, text="SMTP Port:", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=4, column=0, sticky='w', pady=5)
        smtp_port_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35)
        smtp_port_entry.insert(0, str(config.get('email', {}).get('smtp_port', 587)))
        smtp_port_entry.grid(row=4, column=1, pady=5)

        tk.Label(form_frame, text="Sender Email:", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=5, column=0, sticky='w', pady=5)
        sender_email_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35)
        sender_email_entry.insert(0, config.get('email', {}).get('sender_email', ''))
        sender_email_entry.grid(row=5, column=1, pady=5)

        tk.Label(form_frame, text="Sender Password:", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=6, column=0, sticky='w', pady=5)
        sender_password_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35, show='*')
        sender_password_entry.insert(0, config.get('email', {}).get('sender_password', ''))
        sender_password_entry.grid(row=6, column=1, pady=5)

        use_tls_var = tk.BooleanVar(value=config.get('email', {}).get('use_tls', True))
        tk.Checkbutton(form_frame, text="Use TLS", variable=use_tls_var,
                      font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=7, column=1, sticky='w', pady=5)

        # Email Schedule Section
        tk.Label(form_frame, text="⏰ Email Schedule", font=('Segoe UI', 12, 'bold'),
                bg=self.colors['bg'], fg=self.colors['primary']).grid(row=8, column=0, columnspan=2, sticky='w', pady=(20, 10))

        enabled_var = tk.BooleanVar(value=config.get('email_schedule', {}).get('enabled', False))
        tk.Checkbutton(form_frame, text="Enable Daily Email Reports", variable=enabled_var,
                      font=('Segoe UI', 10), bg=self.colors['bg']).grid(row=9, column=0, columnspan=2, sticky='w', pady=5)

        tk.Label(form_frame, text="Send Time (HH:MM):", font=('Segoe UI', 10),
                bg=self.colors['bg']).grid(row=10, column=0, sticky='w', pady=5)
        time_entry = tk.Entry(form_frame, font=('Segoe UI', 10), width=35)
        time_entry.insert(0, config.get('email_schedule', {}).get('send_time', '05:00'))
        time_entry.grid(row=10, column=1, pady=5)

        def save_configuration():
            try:
                new_config = {
                    'anthropic_api_key': api_key_entry.get().strip(),
                    'email': {
                    ...
                    'email_schedule': {
                        ...
                        'send_time': time_entry.get().strip()
                    }
                }
                ...
                if new_config['email_schedule']['enabled']:
                    self.start_email_scheduler()

            except Exception as err:
                messagebox.showerror("Error", f"Failed to save configuration: {err}")
        ...
        def test_email():
            ...
            try:
                ...
                # Create test message
                msg = MIMEMultipart()
                msg['From'] = config_test['email']['sender_email']
                msg['To'] = config_test['email']['sender_email']
                msg['Subject'] = "Sentinel Dashboard - Test Email"

                body = "This is a test email from Sentinel Audit Dashboard. Your email configuration is working correctly!"
                msg.attach(MIMEText(body, 'plain'))

                # Send email
                server = smtplib.SMTP(config_test['email']['smtp_server'], config_test['email']['smtp_port'])
                if config_test['email']['use_tls']:
                    server.starttls()
                server.login(config_test['email']['sender_email'], config_test['email']['sender_password'])
                server.send_message(msg)
                server.quit()

                messagebox.showinfo("Success", "Test email sent successfully!")
                log_message("Test email sent successfully", "INFO")

            except Exception as err:
                messagebox.showerror("Error", f"Failed to send test email: {err}")
                log_message(f"Test email error: {err}", "ERROR")

        # Buttons
        button_frame = tk.Frame(config_window, bg=self.colors['bg'])
        button_frame.pack(fill='x', padx=20, pady=20)

        tk.Button(button_frame, text="💾 Save", bg=self.colors['success'], fg='white',
                 font=('Segoe UI', 10), relief=tk.FLAT, bd=0, padx=20, pady=10,
                 command=save_configuration).pack(side='left', padx=5)

        tk.Button(button_frame, text="📧 Test Email", bg=self.colors['info'], fg='white',
                 font=('Segoe UI', 10), relief=tk.FLAT, bd=0, padx=20, pady=10,
                 command=lambda: run_in_thread(test_email)).pack(side='left', padx=5)

    def send_daily_email_reports(self):
        """Send daily failure reports to all technicians"""
        try:
            config = load_config()
            if not config.get('email', {}).get('sender_email'):
                log_message("Email not configured, skipping daily report", "WARNING")
                return
            ...
            for site_name, db_file in self.shaft_databases_cache.items():
                ...
                # Get technicians for this site
                cursor.execute("SELECT name, email FROM technicians WHERE site_name = ?", (site_name,))
                technicians = cursor.fetchall()
                ...
                # Get failures for today
                today = date.today().strftime('%Y-%m-%d')
                cursor.execute("""
                    SELECT equipment_id, serial, equipment_type, time_tested, gas_type, measured_value
                    FROM sensor_tests
                    WHERE DATE(time_tested) = ? AND outcome = 'fail'
                    ORDER BY time_tested DESC
                """, (today,))
                failures = cursor.fetchall()

                # Get total tests today
                cursor.execute("""
                    SELECT COUNT(*) FROM sensor_tests WHERE DATE(time_tested) = ?
                """, (today,))
                total_tests = cursor.fetchone()[0]
                ...
                # Create email body
                email_body = f"""Sentinel Audit Dashboard - Daily Failure Report
Site: {site_name}
Date: {today}

SUMMARY:
---------
Total Tests Today: {total_tests}
Total Failures Today: {len(failures)}
Failure Rate: {(len(failures)/total_tests*100 if total_tests > 0 else 0):.1f}%
...
"""
                ...
                if failures:
                    for idx, (eq_id, serial, eq_type, time_tested, gas_type, measured_value) in enumerate(failures, 1):
                        email_body += f"{idx}. Equipment ID: {eq_id}\n"
                        email_body += f"   Serial: {serial}\n"
                        email_body += f"   Type: {eq_type}\n"
                        email_body += f"   Time: {time_tested}\n"
                        email_body += f"   Gas: {gas_type}, Value: {measured_value}\n\n"
                else:
                    email_body += "No failures recorded today. Excellent work!\n"

                email_body += f"\n---\nThis is an automated report from Sentinel Audit Dashboard.\n"

                # Send email to each technician
                for tech_name, tech_email in technicians:
                    try:
                        msg = MIMEMultipart()
                        msg['From'] = config['email']['sender_email']
                        msg['To'] = tech_email
                        msg['Subject'] = f"Daily Failure Report - {site_name} - {today}"

                        personalized_body = f"Hello {tech_name},\n\n" + email_body
                        ...
                        log_message(f"Daily report sent to {tech_name} ({tech_email}) for site {site_name}", "INFO")

                    except Exception as err:
                        log_message(f"Failed to send email to {tech_email}: {err}", "ERROR")

            log_message("Daily email reports completed", "INFO")

        except Exception as err:
            log_message(f"Error sending daily reports: {err}", "ERROR")

    def start_email_scheduler(self):
        """Start the email scheduler in a background thread"""
        config = load_config()
        if not config.get('email_schedule', {}).get('enabled'):
            return

        send_time = config.get('email_schedule', {}).get('send_time', '05:00')

        # Clear existing schedule
        schedule.clear()

        # Schedule daily email
        schedule.every().day.at(send_time).do(self.send_daily_email_reports)

        def run_scheduler():
            while config.get('email_schedule', {}).get('enabled'):
        ...
        log_message(f"Email scheduler started - reports will be sent at {send_time}", "INFO")
    ...
    def init_db(self, db_name=None):
        ...
        try:
            ...
            with conn:
                ...
                """)
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_tech_site ON technicians(site_name);")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_tech_email ON technicians(email);")
    ...
    def import_excel(self):
        """Import Excel files into the database with flexible column handling"""
        try:
            db_name = self.get_db_name()
        except ValueError as err:
        ...
        # Only these columns are absolutely required
        required_cols = [
            'equipment_id', 'time_tested', 'outcome'
        ]
        
        # These columns are optional - will be filled with defaults if missing
        optional_cols = {
            'serial': '',
            'equipment_type': '',
            'employee_id': '',
            ...
            with sqlite3.connect(db_name) as conn:
                ...
                for file_idx, file_path in enumerate(file_paths, 1):
                    ...
                    try:
                        ...
                        if missing_optional:
                            log_message(f"File '{file_name}' missing optional columns: {', '.join(missing_optional)} - using defaults", "INFO")
                        
                        rows_before = len(df)
                        
                        # Process time_tested column
                        ...
                        # Show info message if optional columns were missing
                        if missing_optional:
                            self.after(0, lambda fn=file_name, mc=missing_optional: 
                                messagebox.showinfo(
                                    "Column Info",
                                    f"File: {fn}\n\nMissing optional columns were filled with defaults:\n{', '.join(mc)}\n\nImport completed successfully."
                                ))
    ...
    def _get_consolidated_daily_metrics(self):
        ...
        for site_name, db_filename in self.shaft_databases_cache.items():
            ...
            # Count pass/fail per day (treating each equipment once per day)
            daily_counts = daily_equipment_outcome.groupby(['date', 'outcome']).size().unstack(
                fill_value=0
            ).reset_index()
            daily_counts.columns.name = None
